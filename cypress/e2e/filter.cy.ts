describe('Filter Test', () => {
  ...
  it('should find the Vibe buttons', () => {
    cy.get('[for="familyFriendly"]')
      .should('exist')
    cy.get('[for="casual"]')
     .should('exist')
    cy.get('[for="upscale"]')
      .should('exist')
    cy.get('[for="quickEasy"]')
      .should('exist')
    cy.get('[for="lively"]')
      .should('exist')
    cy.get('[for="fun"]')
  ...
  it('should find a toggle for alcohol and for happy hours', () => {
    cy.get(':nth-child(19) > .form-switch > #custom-switch')
     .should('exist')
    cy.get(':nth-child(20) > .form-switch > #custom-switch')
     .should('exist')
  })
  ...
  it('should be able to select options and filter the list', () => {
    ...
    cy.get(':nth-child(5) > div')
     .should('exist')
    cy.get('#card-3 > :nth-child(1)')
     .contains('Cabin Juice')
  })
  ...
  it('should be able to select other options and filter the list', () => {
    ...
    cy.get(':nth-child(5) > div')
     .should('exist')
    cy.get('#card-2 > :nth-child(1)')
      .contains('BoLD Restaurant and Bar')
  })
  ...
  it('should find an error message on the dashboard when no results match', () => {
    ...
    cy.get('.btn-close')
      .click({ force: false })
    cy.get('.fade-in > :nth-child(4) > div')
      .should('exist')
    cy.get('.fade-in > :nth-child(4) > div')
      .contains('No matches for this criteria. Please refine your selections and try again.')
 })
  ...
  it('should be able to use button to change filter options', () => {
    ...
    cy.get('.btn-close')
      .click({ force: false })
    cy.get('.fade-in > :nth-child(4) > div > :nth-child(2)')
      .click()
    cy.wait(250)
    cy.get('[for="cozy"]')
      .click({ multiple: true })
  ...
  it('should be able to use the button to reset all filter options and re-render all cards', () => {
    ...
    cy.get('.fade-in > :nth-child(4) > div')
      .contains('No matches for this criteria. Please refine your selections and try again.')
    cy.get(':nth-child(4) > div > :nth-child(3)')
      .click()
    cy.get('#card-2 > :nth-child(1) > .col-md-8 > .card-body')
      .should('exist')
    cy.get('#card-3 > :nth-child(1) > .col-md-8 > .card-body')
      .should('exist')
    cy.get('#card-1 > :nth-child(1) > .col-md-8 > .card-body')
      .should('exist')
  })
})